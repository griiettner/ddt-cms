...
import type { TestStep, ConfigOption } from '@/types/entities';

// Config type expected by StepRow
interface Config {
  types: ConfigOption[];
  actions: ConfigOption[];
}

function TestCases(): JSX.Element {
  ...
  // Handle scenario name blur
  const handleScenarioNameBlur = (e: FocusEvent<HTMLHeadingElement>): void => {
    if (selectedScenarioId) {
      handleUpdateScenarioName(selectedScenarioId, e.currentTarget.textContent?.trim() || '');
    }
  };

  // Handle scenario name key down
  const handleScenarioNameKeyDown = (e: KeyboardEvent<HTMLHeadingElement>): void => {
    if (e.key === 'Enter') {
      ...
      <div className="main-editor">
        <div className="editor-header">
          <div className="flex items-center gap-2">
            <h2
              className="editable-title text-xl font-bold text-co-blue"
              contentEditable
              suppressContentEditableWarning
              onBlur={handleScenarioNameBlur}
              onKeyDown={handleScenarioNameKeyDown}
            >
              {selectedScenario?.name || 'Select a scenario'}
            </h2>
            {selectedScenarioId && (
              <button className="trash-btn" onClick={openDeleteConfirm} title="Delete Scenario">
          ...
          {isLoading && steps.length === 0 ? (
            ...
            <>
              {steps.length === 0 && selectedScenarioId && (
                <div className="pointer-events-none absolute inset-0 z-10 flex flex-col items-center justify-center bg-gradient-to-b from-white/70 to-white">
                  <div className="-mt-10 text-center">
                    <div className="mb-4 text-4xl">&#10024;</div>
                    <h3 className="mb-2 text-2xl font-bold text-co-blue">
                      Zero steps? No problem.
                    </h3>
                    <p className="text-co-gray-500">
                      This scenario is a blank canvas. Let&apos;s add your first test step.
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                <table className="steps-table">
                  ...
                  <tbody>
                    {steps.length === 0 && !selectedScenarioId ? (
                      <tr>
                        <td colSpan={9} className="text-co-gray-400 py-20 text-center italic">
