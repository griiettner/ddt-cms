...
import type { TableColumnInfo } from '../types/index.js';
...
export const initSchema = async (): Promise<void> => {
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_releases_status ON releases(status)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_runs_release ON test_runs(release_id)`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_runs_executed ON test_runs(executed_at DESC)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_select_configs_name ON select_configs(name)`);
  await db.exec(
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_match_configs_name ON match_configs(name)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_categories_path ON categories(path)`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_categories_lft_rgt ON categories(lft, rgt)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_reusable_cases_name ON reusable_cases(name)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_sets_release ON test_sets(release_id)`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_sets_category ON test_sets(category_id)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_cases_release ON test_cases(release_id)`);
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_cases_set ON test_cases(test_set_id)`);
  await db.exec(
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_steps_release ON test_steps(release_id)`);
  await db.exec(
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_audit_logs_user ON audit_logs(user_eid)`);
  await db.exec(
    `CREATE INDEX IF NOT EXISTS idx_audit_logs_resource ON audit_logs(resource_type, resource_id)`
  );
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_audit_logs_release ON audit_logs(release_id)`);
  ...
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_run_steps_run ON test_run_steps(test_run_id)`);
  await db.exec(
    `CREATE INDEX IF NOT EXISTS idx_test_run_steps_scenario ON test_run_steps(scenario_id)`
  );
  await db.exec(`CREATE INDEX IF NOT EXISTS idx_test_run_steps_status ON test_run_steps(status)`);
 ...
 */
const runMigrations = async (db: DatabaseWrapper): Promise<void> => {
  try {
    // Migration: Add new columns to test_runs if they don't exist
    const testRunsColumns = await db.all<TableColumnInfo>('PRAGMA table_info(test_runs)');
    const testRunsColumnNames: string[] = testRunsColumns.map((c) => c.name);

    if (!testRunsColumnNames.includes('test_set_name')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN test_set_name VARCHAR(255)');
    }
    if (!testRunsColumnNames.includes('total_scenarios')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN total_scenarios INTEGER DEFAULT 0');
    }
    if (!testRunsColumnNames.includes('total_steps')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN total_steps INTEGER DEFAULT 0');
    }
    if (!testRunsColumnNames.includes('passed_steps')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN passed_steps INTEGER DEFAULT 0');
    }
    if (!testRunsColumnNames.includes('failed_steps')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN failed_steps INTEGER DEFAULT 0');
    }
    if (!testRunsColumnNames.includes('failed_details')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN failed_details TEXT');
    }

    // Migration: Add release_id to test_cases if not present (for legacy data)
    const testCasesColumns = await db.all<TableColumnInfo>('PRAGMA table_info(test_cases)');
    const testCasesColumnNames: string[] = testCasesColumns.map((c) => c.name);

    if (!testCasesColumnNames.includes('release_id')) {
      await db.exec('ALTER TABLE test_cases ADD COLUMN release_id INTEGER');
      await db.exec('CREATE INDEX IF NOT EXISTS idx_test_cases_release ON test_cases(release_id)');
    }

    // Migration: Add release_id to test_scenarios if not present
    const testScenariosColumns = await db.all<TableColumnInfo>('PRAGMA table_info(test_scenarios)');
    const testScenariosColumnNames: string[] = testScenariosColumns.map((c) => c.name);

    if (!testScenariosColumnNames.includes('release_id')) {
      await db.exec('ALTER TABLE test_scenarios ADD COLUMN release_id INTEGER');
      await db.exec(
        'CREATE INDEX IF NOT EXISTS idx_test_scenarios_release ON test_scenarios(release_id)'
      );
    }
    ...
    const testStepsColumns = await db.all<TableColumnInfo>('PRAGMA table_info(test_steps)');
    const testStepsColumnNames: string[] = testStepsColumns.map((c) => c.name);

    if (!testStepsColumnNames.includes('release_id')) {
      await db.exec('ALTER TABLE test_steps ADD COLUMN release_id INTEGER');
      await db.exec('CREATE INDEX IF NOT EXISTS idx_test_steps_release ON test_steps(release_id)');
    }

    // Migration: Add release_id to configuration_options if not present
    const configColumns = await db.all<TableColumnInfo>('PRAGMA table_info(configuration_options)');
    const configColumnNames: string[] = configColumns.map((c) => c.name);

    if (!configColumnNames.includes('release_id')) {
      await db.exec('ALTER TABLE configuration_options ADD COLUMN release_id INTEGER');
      await db.exec(
        'CREATE INDEX IF NOT EXISTS idx_config_release ON configuration_options(release_id)'
      );
    }
    ...
    if (!testRunsColumnNames.includes('environment')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN environment VARCHAR(50)');
    }

    // Migration: Add base_url column to test_runs if not present
    if (!testRunsColumnNames.includes('base_url')) {
      await db.exec('ALTER TABLE test_runs ADD COLUMN base_url TEXT');
    }

    // Migration: Add expected_results column to test_run_steps if not present
    const testRunStepsColumns = await db.all<TableColumnInfo>('PRAGMA table_info(test_run_steps)');
 ...
 */
export const initRegistrySchema = async (): Promise<void> => {
  await initSchema();
};
 ...
 */
export const initReleaseSchema = async (_dbPath: string): Promise<void> => {
